...
public class AutoAdapter extends BaseAdapter implements Filterable {
	...
	public AutoAdapter(Context context, ArrayList<JSONObject> _games, int _layoutResourceId) {
		_context = context;
	...
	public String getItem(int arg0) {
		try {
			return games.get(arg0).getString("ProductCode");
		} catch (Exception e) {
	...
	public long getItemId(int arg0) {
		try {
			return games.get(arg0).getInt("ProductId");
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		try {
			textViewItem.setText(objectItem.getString("ProductCode") + "." + objectItem.getString("ProductName"));
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
			...
			if (constraint != null) {
				if (orig != null && orig.size() > 0) {
					for (JSONObject g : orig) {
						try {
							if (g.getString("ProductCode").toLowerCase().contains(constraint.toString().toLowerCase())
									|| g.getString("ProductName").toLowerCase()
