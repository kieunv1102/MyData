...
public class ProductGridChange extends BaseAdapter {
	...
	public ProductGridChange(Context c, List<JSONObject> d) {
		mContext = c;
		dataSource = d;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		try {
			...
			// thay doi item
			holder.editSoluongNew.setText(lineSource.getString("SLnew"));
			...
			holder.editSoluongNew.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					try {
						editText1.setText(lineSource.getString("SLnew"));
						...
						public void onClick(View arg0) {
							...
							if (Util.checkOffline == true) {
								...
								try {
									...
									if (x > 9999) {
										dataSource.get(position).put("SLnew", String.valueOf(9999));
									} else {
										dataSource.get(position).put("SLnew", String.valueOf(x));
									}
									holder.txtThanhTien.setText(String.valueOf((long) lineSource.getDouble("Price")
											* lineSource.getInt("SLnew") * (lineSource.getInt("KMnew") == 1 ? 0 : 1)));
									updateFooter();
								...
								try {
									...
									if (!validateQuantity(position, dataSource.get(position).getString("KMnew"), x,
											lineSource)) {
										...
										if (Util.checkSaveOffline == true) {
											...
											for (int i = 0; i < lstJsonObjects.size(); i++) {
												if (dataSource.get(position).getString("ProductCode")
														.equals(lstJsonObjects.get(i).getString("ProductCode"))) {
													sl = lstJsonObjects.get(i).getInt("Quantity");
													km = lstJsonObjects.get(i).getInt("Promotion");
												}
											}
											Toast.makeText(mContext, "Không còn đủ số lượng, Tồn kho thực tế: " + sl
													+ ",khuyến mại:" + km, Toast.LENGTH_SHORT).show();

										}

										else
											Toast.makeText(mContext, "Không còn đủ số lượng, Tồn kho thực tế: "
													+ dataSource.get(position).getInt("Quantity") + ",khuyến mại:"
													+ dataSource.get(position).getInt("Promotion"), Toast.LENGTH_SHORT)
													.show();
									...
									holder.editSoluong.setText(lineSource.getString("SLnew"));
									holder.txtThanhTien.setText(String.valueOf((long) lineSource.getDouble("Price")
											* lineSource.getInt("SLnew") * (lineSource.getInt("KMnew") == 1 ? 0 : 1)));
									updateFooter();
			...
			double ttnew = (double) lineSource.getDouble("Price") * lineSource.getInt("SLnew")
					* (lineSource.getInt("KMnew") == 1 ? 0 : 1);

			holder.txtThanhTien.setText(FormatUtil.formatCurrency(tt));
			holder.txtThanhTien.setTag(position);

			holder.txtThanhTienNew.setText(FormatUtil.formatCurrency(ttnew));
			holder.txtThanhTienNew.setTag(position);

			// holder.chkKM.setText(lineSource.getString("KM"));
			holder.chkKMNew.setTag(position);
			holder.chkKMNew.setChecked(lineSource.getString("KMnew").equals("1"));
			if (lineSource.getString("IsLot").equalsIgnoreCase("true")) {
				holder.chkKMNew.setEnabled(false);
			} else {
				holder.chkKMNew.setEnabled(true);
			}
			...
			holder.chkKMNew.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					try {
						...
						if (!validateQuantity(position, (isChecked ? "1" : "0"), lineSource.getInt("SLnew"),
								lineSource)) {
						...
						dataSource.get(position).put("KMnew", isChecked ? 1 : 0);
						// ko khuyển mại trỏ về khuyến mại thì xóa đi
						if (!isChecked) {
							if (countIndex >= 2) {
								dataSource.remove(position);
							} else {
								holder.txtThanhTien.setText(String
										.valueOf((int) lineSource.getDouble("Price") * lineSource.getInt("SLnew")));
								dataSource.get(position).put("TTnew", String
										.valueOf((int) lineSource.getDouble("Price") * lineSource.getInt("SLnew")));
							}

						} else {
							holder.txtThanhTienNew.setText("0");
							dataSource.get(position).put("TTnew", 0);
						}
	...
	void updateFooter() {
		...
		for (int jx = 0; jx <= getDataSource().size() - 1; jx++) {
			try {
				...
				if (getDataSource().get(jx).getString("KMnew").equals("1")) {
					getDataSource().get(jx).put("TTnew", 0);
				} else {
					getDataSource().get(jx).put("TTnew",
							getDataSource().get(jx).getDouble("Price") * getDataSource().get(jx).getInt("SLnew"));
					total = total + getDataSource().get(jx).getLong("TTnew");
				}
