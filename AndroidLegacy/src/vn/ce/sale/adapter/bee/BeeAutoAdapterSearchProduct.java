...
public class BeeAutoAdapterSearchProduct extends BaseAdapter implements Filterable {

	Context _context;
	ArrayList<JSONObject> games;
	int layoutResourceId = -1;

	public BeeAutoAdapterSearchProduct(Context context, ArrayList<JSONObject> _games, int _layoutResourceId) {
		_context = context;
	...
	public String getItem(int arg0) {
		try {
			return games.get(arg0).getString("Name");
		} catch (Exception e) {
	...
	public long getItemId(int arg0) {
		try {
			return games.get(arg0).getInt("Id");
			...
			if (constraint != null) {
				if (orig != null && orig.size() > 0) {
					for (JSONObject g : orig) {
						try {
							if (g.getString("Name").toLowerCase().contains(constraint.toString().toLowerCase()))
								results.add(g);
						} catch (JSONException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
