...
public class FormatUtil {
	public static void main(String[] args) throws ParseException {
		// int tongtien = (50000);//2200.0;
		// System.out.print(formatCurrency(tongtien));

		double tongtien = (50000);// 2200.0;
	...
	public static String formatNumberForRead(double number) {
		...
		DecimalFormat formatter = new DecimalFormat("###,###,###.##");
		String strReturn = formatter.format(number);
		strReturn = strReturn.replace(",", "");
		strReturn = strReturn.replace(".", "");
		return strReturn;
		/*
		 * NumberFormat nf = NumberFormat.getInstance(); String temp =
		 * nf.format(number); String strReturn = ""; int slen = temp.length();
		 * for (int i = 0; i < slen; i++) { if
		 * (String.valueOf(temp.charAt(i)).equals(".")) break; else if
		 * (Character.isDigit(temp.charAt(i))) { strReturn +=
		 * String.valueOf(temp.charAt(i)); } } return strReturn;
		 */

	}

	/*
	 * public static String numberToStringNew(double number) { ArrayList<String>
	 * kq = FormatUtil.readNum(String.valueOf(number)); StringBuilder sBuilder=
	 * new StringBuilder(); for (int i = 0; i < kq.size(); i++) {
	 * sBuilder.append(kq.get(i)+ " "); } return sBuilder.toString(); }
	 */
	public static String numberToString(double number) {
		// if(true)return numberToStringNew(number);
